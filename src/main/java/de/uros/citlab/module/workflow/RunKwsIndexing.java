 ...
 * @author gundram
 */
public class RunKwsIndexing extends ParamTreeOrganizer {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = LoggerFactory.getLogger(RunKwsIndexing.class.getName());
...
//    private int threads = 1;
    ...
    public RunKwsIndexing(String htr, String lr, String la, String folderIn, String folderOut, boolean createDebugImg, boolean saveCM, String[] props) {
        this(htr, lr, la, folderIn, folderOut, null, createDebugImg, saveCM, props);
    }

    public RunKwsIndexing(String htr, String lr, String la, String folderIn, String folderOut, String b2pClassName, boolean createDebugImg, boolean saveCM, String[] props) {
...
//            props = PropertyUtil.setProperty(props, Key.LA_ROTSCHEME, "default");
        ...
        this.saveCM = saveCM;
        addReflection(this, RunKwsIndexing.class);
    }
...
//            String htr = null;
            ...
            al.addArgument("xml_in", HomeDir.getFile("data/speed"));
            al.addArgument("xml_out", HomeDir.getFile("index/speed/" + htr));
            al.addArgument("htr_path", HomeDir.getFile("nets/" + htr + "/net.sprnn"));
//            al.setHelp();
            args = al.getArgs();
        }

        String folder = "", folderOut = "", htr = "", lr = "", la = null, b2p = B2PSeamMultiOriented.class.getCanonicalName();
        String[] props = null;
//        props = PropertyUtil.setProperty(props, Key.LA_SEPSCHEME, LayoutAnalysisURO_ML.SEP_NEVER);
//        props = PropertyUtil.setProperty(props, Key.LA_DELETESCHEME, LayoutAnalysisURO_ML.DEL_LINES);
        RunKwsIndexing instance = new RunKwsIndexing(htr, lr, la, folder, folderOut, b2p, false, true, props);
...
//        System.out.println(Arrays.toString(remainingArgumentList));
        ...
        instance = new RunKwsIndexing(htr, lr, la, folder, folderOut, b2p, false, true, props);
        ps = new ParamSet();
        ps.setCommandLineArgs(args);    // allow early parsing
        ps = instance.getDefaultParamSet(ps);
        ps = ParamSet.parse(ps, args, ParamSet.ParseMode.ACCEPT); // be strict, don't accept generic parameter
